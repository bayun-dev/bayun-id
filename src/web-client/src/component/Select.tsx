...
const Select = (props: PropsWithChildren<SelectProps>) => {
    ...
    const id = props.id ? props.id : nextId('sel-')
    ...
    return <>
        <div>
            ...
            <select id={id} value={props.value} className={iClasses} disabled={props.disabled} onChange={props.onChange}>
                {props.values && <>
                {Object.keys(props.values).map((value, index, array) => {
                    return <option key={id+'-'+index} value={value}>{props.values?.[value]}</option>
                })}
                </>}
            </select>
            { props.errorMessage && <p className="mt-2 text-sm text-red-600 dark:text-red-500">{props.errorMessage}</p> }
            { props.children && <p id="helper-text-explanation" className="mt-2 text-sm text-gray-500 dark:text-gray-400">{props.children}</p> }
        </div>
    </>
}
